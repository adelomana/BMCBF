...
results_dir = '/Users/adrian/research/bmcbf/055_bruara/results/transcriptomics/degs'
...
# 2. define metadata
#
### in this experiment, look for a samples as they are --rf-stranded
...
dds$genotype = relevel(dds$genotype, ref="DMSO")
...
df = data.frame(plotting_x=clip(plotting_x, .min=-3, .max=3), plotting_y=clip(plotting_y, .min=0, .max=20), plotting_z=clip(z, .min=0, .max=2))
...
ggplot() + 
  geom_point(data=reds, aes(x=plotting_x, y=plotting_y, color=plotting_z), , size=5, shape=19, alpha=3/3, stroke=0) + 
  geom_point(data=blues, aes(plotting_x, plotting_y, color=plotting_z), size=5, shape=19, alpha=3/3, stroke=0) +
  geom_point(data=blacks, aes(plotting_x, plotting_y), color = "black", size=3, shape=19, alpha=0.2, stroke=0) +
  ...
  geom_segment(aes(x=1, xend=1, y=-log10(0.05), yend=20), linetype=2) +
  geom_segment(aes(x=-3, xend=-1, y=-log10(0.05), yend=-log10(0.05)), linetype=2) +
