...
def launcher(sample):
    ...
    working_files = [element for element in all_files if 'fastq' in element]
    working_files.sort()
    working_label = working_files[0].split('_')[0]
    ...
    options = ' -i {} -o kallisto_output_reverse -t {} -b 100 --rf-stranded --verbose '.format(transcriptome_index, number_threads)
    kallisto_cmd_a = executable + options + fastq_files_string

    options = ' -i {} -o kallisto_output_forward -t {} -b 100 --fr-stranded --verbose '.format(transcriptome_index, number_threads)
    kallisto_cmd_b = executable + options + fastq_files_string

    options = ' -i {} -o kallisto_output_unstranded -t {} -b 100 --verbose '.format(transcriptome_index, number_threads)
...
#
raw_fastq_dir = '/hpcdata/Mimir/adrian/research/038_husavik/data/' 
...
output_dir = '/hpcdata/Mimir/adrian/research/038_husavik/results/'
