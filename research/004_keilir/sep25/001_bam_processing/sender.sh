...
for bam in "$PARENT_DIR"/*/*.bam; do
  ...
  filter_cmd="time samtools view --threads 8 -b -q 30 $dup_bam > $out"
  echo "$filter_cmd"
  eval "$filter_cmd"
  ...
  cmd1="time samtools sort -n -@ 8 -o $qname_bam $out"
  cmd2="time bedtools bamtobed -bedpe -i $qname_bam > $bedpe"
  cmd3="time awk '\$1==\$4 && \$6-\$2 < 1000 {print \$0}' $bedpe > $clean"
  cmd4="time cut -f 1,2,6 $clean | sort -k1,1 -k2,2n -k3,3n > $fragments"
