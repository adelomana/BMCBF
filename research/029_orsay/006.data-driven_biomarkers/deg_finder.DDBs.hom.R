...
results_dir = '/Users/adrian/research/bmcbf/029_orsay/results/006.DDBs'
...
genotypes = c(rep('other', 9), rep('other', 9), rep('HOM', 12), rep('other', 4))
metadata = data.frame(labels)
metadata$path = paths
metadata$genotype = genotypes

metadata = metadata[!(metadata$labels %in% c("P9T001","P9T004", "P9T009")),]
dim(metadata)
View(metadata)

seta_indexes = 16:27
setb_indexes = c(c(1:5), c (28:31))
...
dds$genotype = relevel(dds$genotype, ref="HOM")
...
cat(blue(paste('all vs HOM:', dim(filtred_results)[1], sep=' ')), fill=TRUE)
...
rownames(subset) = wo
sorted_filtred_results$medianTPM_HOM = rowMedians(subset[rownames(sorted_filtred_results), seta_indexes]) # HOM
sorted_filtred_results$medianTPM_all = rowMedians(subset[rownames(sorted_filtred_results), setb_indexes]) # others

write.table(sorted_filtred_results, file=paste(results_dir, '/DDBs_HOM.for.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect all vs HOM')
