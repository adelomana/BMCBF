...
genotypes = c(rep('other', 9), rep('HET', 9), rep('other', 12), rep('other', 4))
metadata = data.frame(labels)
metadata$path = paths
...
cat(blue(paste('all vs HET:', dim(filtred_results)[1], sep=' ')), fill=TRUE)
...
rownames(subset) = wo
sorted_filtred_results$medianTPM_HET = rowMedians(subset[rownames(sorted_filtred_results), seta_indexes]) # HET
sorted_filtred_results$medianTPM_all = rowMedians(subset[rownames(sorted_filtred_results), setb_indexes]) # others

write.table(sorted_filtred_results, file=paste(results_dir, '/DDBs_HET.for.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect all vs HET')
