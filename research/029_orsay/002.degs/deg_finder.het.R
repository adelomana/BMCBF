...
sorted_filtred_results$medianTPM_HET = rowMedians(subset[rownames(sorted_filtred_results), 10:18]) # HET

write.table(sorted_filtred_results, file=paste(results_dir, '/effect_HET_vs_WT.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_HET_vs_WT.anti.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect HET vs WT')
