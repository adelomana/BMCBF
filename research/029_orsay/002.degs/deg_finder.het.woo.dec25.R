...
mini = metadata[metadata$genotype == 'WT' | metadata$genotype == 'HET', ]
# excluded samples from immuno analysis are: 
immuno_excluded <- c("P9T001","P9T002","P9T004","P9T008","P9T009","P9T011","P9T015","P9T017","P9T022","P9T029","P9T034")
metadata = mini[!(mini$labels %in% immuno_excluded),]
dim(metadata)
View(metadata)

seta_indexes = 1:sum(metadata$genotype == 'WT')
setb_indexes = (sum(metadata$genotype == 'WT')+1):length(metadata$genotype)
seta_indexes
setb_indexes
...
cat(blue(paste('HET vs WT:', dim(filtred_results)[1], sep=' ')), fill=TRUE)
...
write.table(sorted_filtred_results, file=paste(results_dir, '/effect_HET_vs_WT.woo.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_HET_vs_WT.woo.anti.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect HET vs WT woo')
