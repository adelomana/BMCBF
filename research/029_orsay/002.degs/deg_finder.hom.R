...
a = rowMedians(subset[ , 1:9])
b = rowMedians(subset[ , 10:21])
c = pmax(a, b)
...
rownames(subset) = wo
sorted_filtred_results$medianTPM_WT = rowMedians(subset[rownames(sorted_filtred_results), 1:9]) # WT
sorted_filtred_results$medianTPM_HET = rowMedians(subset[rownames(sorted_filtred_results), 10:21]) # HET

write.table(sorted_filtred_results, file=paste(results_dir, '/effect_HOM_vs_WT.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_HOM_vs_WT.anti.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect HOM vs WT')
