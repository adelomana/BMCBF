...
mini = metadata[metadata$genotype == 'WT' | metadata$genotype == 'HOM', ]
...
cat(blue(paste('HOM vs WT:', dim(filtred_results)[1], sep=' ')), fill=TRUE)
...
write.table(sorted_filtred_results, file=paste(results_dir, '/effect_HOM_vs_WT.woo.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_HOM_vs_WT.woo.anti.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect HOM vs WT woo')
