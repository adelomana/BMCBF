...
results_dir = '/Users/adrian/research/bmcbf/031_reykjavik/results'

effect_size_threshold = log2(2)
face_value_threshold = 50
...
#
# 5. filter out genes with a median difference smaller than 50 reads
#
...
sorted_filtred_results$annotation1 = subu[rownames(sorted_filtred_results), 'ensembl_gene_id']

write.table(sorted_filtred_results, file=paste(results_dir, '/effect_right_vs_left_all.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_right_vs_left_all.anti.tsv', sep=''), quote=FALSE, sep='\t')
