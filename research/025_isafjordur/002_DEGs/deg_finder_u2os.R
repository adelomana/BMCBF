...
write.table(sorted_filtred_results, file=paste(results_dir, '/effect_ko_vs_wt.u2os.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_ko_vs_wt.u2os.anti.tsv', sep=''), quote=FALSE, sep='\t')
...
ggplot() + 
  ...
  labs(x=expression('Expression [log'[2]~'FC]'), y=expression('Significance [log'[10]~'adjusted P]'), color=expression('Expression [log'[10]~'TPM]'), title='KO vs WT | U2OS') + 
  theme_linedraw() +
