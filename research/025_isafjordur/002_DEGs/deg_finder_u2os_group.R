...
initial_paths = file.path(dirnames, 'kallisto_output_c/abundance.h5')
exclusion_samples = c('U2-1_Sc_1h', 'U2-1_Sc_4h', 'U2-1_siMi_1h', 'U2-1_siMi_4h', 'U2-1_siMi_UT', 'U2-2_Sc_1h', 'U2-2_Sc_UT')
paths = c()
for (i in 1:length(initial_paths))
{
  print(i)
  print(initial_paths[i])
  adding = TRUE
  for (j in 1:length(exclusion_samples))
  {
    if (grepl(exclusion_samples[j], initial_paths[i]) == TRUE) {adding=FALSE}
  }
  if (adding == TRUE) {paths = c(paths, initial_paths[i])}
}
paths
...
genotypes = c(rep('wt', 2), rep('ko', 3), rep('wt', 3), rep('ko', 3))
...
a = counts(dds)[ , which(genotypes == 'wt')]
b = counts(dds)[ , which(genotypes == 'ko')]
...
a = rowMedians(subset[ , which(genotypes == 'wt')])
b = rowMedians(subset[ , which(genotypes == 'ko')])
c = pmax(a, b)
...
write.table(sorted_filtred_results, file=paste(results_dir, '/effect_ko_vs_wt.u2os.group.for.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/effect_ko_vs_wt.u2os.group.anti.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('genotype')) + ggtitle('effect ko vs wt | group')
...
ggplot() + 
  ...
  labs(x=expression('Expression [log'[2]~'FC]'), y=expression('Significance [log'[10]~'adjusted P]'), color=expression('Expression [log'[10]~'TPM]'), title='KO vs WT | U2OS group') + 
  theme_linedraw() +
