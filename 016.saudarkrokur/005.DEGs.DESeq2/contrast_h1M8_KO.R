...
#
# 3.1. contrast h1M8 vs KO
#
rule = (metadata$condition == 'ko') | (metadata$condition == 'h1M8')
...
write.table(sorted_filtred_results, 
            file=paste(results_dir, '/effect_h1M8_vs_KO.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, 
            file=paste(results_dir, 
                       '/effect_h1M8_vs_KO.anti.tsv', sep=''), quote=FALSE, sep='\t')

plotPCA(rlog(dds), intgroup=c('condition')) + ggtitle('effect h1M8 vs KO')
